...
class Game {
    ...
    void decode(ENetPacket* packet) {
        ...
        int i = 0;
        while (packet->dataLength > i) { // something remains to be seen in the packet
        printf("packet decode while loop \n");
            switch (packet->data[i]) {
                ...
                case 'D':
                    ...
                    int c = 0, index;
                    while (packet->data[i] != '\n') {
                        index = packet->data[i]-'0'; // assuming that the indices are 1 digit long
                        i += 2;
                        if (packet->data[i] == 'm') {
                            i += 2;
                            c = 0;
                        ...
                        if (packet->data[i] == 'p') {
                            i += 2;
                            dogs[index].set_disappear(packet->data[i]-'0');
                            i+=2;
                        }
                        if (packet->data[i] == 'd') {
                            i += 2;
                            dogs[index].set_direction(packet->data[i]-'0');
                            i+=2;
                        }
                        if (packet->data[i] == 'x') {
                            i += 2;
                ...
                case 'G':
                    ...
                    while (packet->data[i] != '\n') {
                        ...
                        if (packet->data[i] == 'p') {
                            i += 2;
                            guards[index].set_disappear(packet->data[i]-'0');
                            i+=2;
                        }
                        if (packet->data[i] == 'd') {
                            i += 2;
                            guards[index].set_direction(packet->data[i]-'0');
                            i+=2;
                        }
                        if (packet->data[i] == 'x') {
                            i += 2;
        ...
        while (enet_host_service(client, &event, 0) > 0) {
            switch(event.type) {
                ...
                case ENET_EVENT_TYPE_RECEIVE:
                    printf("received a packet from server\n");
