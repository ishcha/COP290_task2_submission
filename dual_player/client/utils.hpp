...
bool enet_initialization(ENetAddress& address, ENetHost*& client, ENetEvent& e, ENetPeer*& peer) {
    ...
    if (enet_host_service (client, & e, 5000) > 0 && e.type == ENET_EVENT_TYPE_CONNECT) {
        ...
        printf ("Connection to 127.0.0.1:1234 failed.");
        return false;
    }
    return true;
}

void close(SDL_Window*& window, SDL_Renderer*& renderer, TTF_Font*& font, ENetHost*& client, ENetPeer*& peer) {
    TTF_CloseFont(font);
    font = NULL;
    TTF_Quit();
