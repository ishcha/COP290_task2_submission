    ...
    string handleEvent( SDL_Event& e, unsigned char image[sheight][swidth][BYTES_PER_PIXEL], SDL_Renderer*& renderer) {
        if( e.type == SDL_KEYDOWN) {
            switch( e.key.keysym.sym ) {
                case SDLK_UP: return update(x, y-vy, image, renderer); break;
                case SDLK_DOWN: return update(x, y+vy, image, renderer); break;
                case SDLK_LEFT: return update(x-vx, y, image, renderer); break;
                case SDLK_RIGHT: return update(x+vx, y, image, renderer); break;
            }
        }
        return "";
    }
...
class Game {
    ...
    Game() {
        ...
        makePaths(image);
        dog_update = new int[NDOGS];
        guard_update = new int[NGUARDS];
    }
    ...
    string handleEvent(SDL_Event e, SDL_Renderer*& renderer) {return "P " + p1.handleEvent(e, image, renderer);}

    void draw_key(SDL_Renderer*& renderer) {
        if (p1.now_has_key()) return;
    ...
    string show_move_obstacles_stations_goal(SDL_Renderer*& renderer) {
        ...
        for(int i=0; i<NDOGS; i++) if (dog_update[i]%10 == 0) changes << "D " << i << ' ' <<dogs[i].move(image, p1, p2);
        for(int i=0; i<NGUARDS; i++) if (guard_update[i]%15 == 0) changes << "G " << i << ' ' <<guards[i].move(image, p1, p2);
    ...
    void decode(ENetPacket* packet) {
        if (packet->dataLength <= 4) return;
        int i = 2;
        if (packet->data[i] == 'x') {
            i+=2;
            int c = 0;
            while (isdigit(packet->data[i])){
...
int main() {
    ...
    if (!enet_initialization(address, server)) {close(window, renderer, font); enet_deinitialize(); return 1;}

    Game* game = new Game();
    game->Game_init(); // static changes
    game->fill_dog_update(); // server only
    game->fill_guard_update(); // server only
    bool cutoff = false;
    ...
    while (!quit) {
        ...
        while (SDL_PollEvent(&e) != 0) {
            ...
            if (window_focus) changes += game->handleEvent(e, renderer); // dynamic done!
        }
        SDL_SetRenderDrawColor( renderer, GREEN_BG[R], GREEN_BG[G], GREEN_BG[B], 0xFF );
        SDL_RenderClear(renderer);
        if (!load_game_screen(window, renderer)) {quit = true;} // static changes
        ...
        if (peer != NULL) {
            game->update_obstacle_update(); // server only!

            game->draw_key(renderer); // static changes
            game->draw_particles(renderer, font); // static changes

            changes += game->show_move_obstacles_stations_goal(renderer); // dynamic done!

            SDL_RenderPresent(renderer);
            game->check_report_result(renderer, quit, font); // quit will be done individually
    ...
    close(window, renderer, font, server);
    enet_deinitialize();
    return 0;
}