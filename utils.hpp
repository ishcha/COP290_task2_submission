...
void close(SDL_Window*& window, SDL_Renderer*& renderer, TTF_Font*& font, Mix_Chunk*& sounds) {
    for(int i = 0; i< 5; i++) {Mix_FreeChunk( &sounds[i] );}
    Mix_Quit();
    TTF_CloseFont(font);
    font = NULL;
    TTF_Quit();
...
void lost_message(SDL_Renderer*& renderer, TTF_Font* font, Mix_Chunk* lost) {
...
void win_message(SDL_Renderer*& renderer, TTF_Font* font, Mix_Chunk* won) {
...
Mix_Chunk* initialize_music() {
    Mix_Chunk* all_effects = new Mix_Chunk[5];
    all_effects[0] = *Mix_LoadWAV("music/acquiring_key.wav");
    all_effects[1] = *Mix_LoadWAV("music/energizing.wav");
    all_effects[2] = *Mix_LoadWAV("music/Game_win.wav");
    all_effects[3] = *Mix_LoadWAV("music/gamelose.wav");
    all_effects[4] = *Mix_LoadWAV("music/mild_obstacle.wav");
    return all_effects;
}